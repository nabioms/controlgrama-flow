...
import { CheckCircle2, ClipboardList, Plus, Settings2, UsersRound, XCircle } from "lucide-react";
...
function ServiceOrdersPage() {
  ...
  const [date, setDate] = useState(today), [typeId, setTypeId] = useState(""), [contractId, setContractId] = useState(""), [teamId, setTeamId] = useState(""), [quantity, setQuantity] = useState(""), [notes, setNotes] = useState("");
  const [showTypes, setShowTypes] = useState(false), [typeName, setTypeName] = useState(""), [typeUnit, setTypeUnit] = useState<ServiceUnit>("m2"), [typePrice, setTypePrice] = useState("");
  const [filterMonth, setFilterMonth] = useState(today.slice(0, 7)), [filterType, setFilterType] = useState("all"), [filterStatus, setFilterStatus] = useState("all"), [filterTeam, setFilterTeam] = useState("all");
  ...
  const filteredOrders = useMemo(
    () => serviceOrders.filter((o) => o.service_date.slice(0, 7) === filterMonth && (filterType === "all" || o.service_type_id === filterType) && (filterStatus === "all" || o.status === filterStatus) && (filterTeam === "all" || o.team_id === filterTeam)),
  ...
  async function createOrder() {
    ...
    try {
      await addServiceOrder({ service_date: date, service_type_id: typeId, contract_id: contractId || null, team_id: teamId, planned_quantity: Number(quantity), notes: notes || null });
      setQuantity(""); setNotes(""); setTeamId("");
    } catch (e: any) {
  ...
  async function createType() {
    setError("");
    try { await addServiceType(typeName, typeUnit, Number(typePrice)); setTypeName(""); setTypePrice(""); }
  ...
  return (
    <AppShell title="Ordens de serviço" subtitle="Produção, histórico e equipe executora">
      ...
      <Card className="mb-5">
        <SectionTitle title="Abrir O.S." hint="Escolha a equipe já cadastrada para esta frente de serviço." />
        ...
        <Button className="mt-3 w-full" disabled={saving || !typeId || !teamId || Number(quantity) <= 0} onClick={createOrder}><Plus className="size-4" /> Abrir O.S.</Button>
      </Card>
          ...
          return (
            <Card key={o.id} className="p-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div><p className="text-[11px] text-muted-foreground">Equipe</p><p className="font-semibold text-xs">{team?.name || "Não informada"}</p><p className="text-[11px] text-muted-foreground">{team?.foreman?.full_name ? `Enc.: ${team.foreman.full_name}` : "Sem encarregado"}</p></div>
                <div className="text-right"><p className="text-[11px] text-muted-foreground">{o.status === "realizada" ? "Realizado" : "Valor da O.S."}</p><p className="font-semibold">{brl(Number(o.status === "realizada" ? o.realized_amount : o.planned_amount))}</p></div>
              </div>
