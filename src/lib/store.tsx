...
export function StoreProvider({children}:{children:ReactNode}){
 ...
 const addTeam=useCallback(async(name:string,foremanWorkerId:string|null)=>{const clean=name.trim();if(!clean)throw new Error("Informe o nome da equipe.");if(foremanWorkerId){const foreman=workers.find(w=>w.id===foremanWorkerId);if(!foreman)throw new Error("Encarregado não encontrado.");if(foreman.job_role!=="encarregado")throw new Error("O responsável da equipe precisa estar cadastrado como encarregado.");}const {data,error}=await supabase.from("teams").insert({name:clean,foreman_worker_id:foremanWorkerId||null}).select("*").single();if(error)throw error;setTeams(x=>[({...data,foreman:foremanWorkerId?workers.find(w=>w.id===foremanWorkerId)||null:null,members:[]}) as Team,...x])},[workers]);
 const updateTeam=useCallback(async(id:string,patch:Partial<Pick<Team,"name"|"foreman_worker_id"|"active">>)=>{if(patch.foreman_worker_id){const foreman=workers.find(w=>w.id===patch.foreman_worker_id);if(!foreman||foreman.job_role!=="encarregado")throw new Error("O responsável da equipe precisa estar cadastrado como encarregado.");}const {data,error}=await supabase.from("teams").update(patch).eq("id",id).select("*").single();if(error)throw error;setTeams(x=>x.map(t=>t.id===id?({...t,...data,foreman:data.foreman_worker_id?workers.find(w=>w.id===data.foreman_worker_id)||null:null}):t))},[workers]);
 const setTeamMembers=useCallback(async(teamId:string,workerIds:string[])=>{const unique=[...new Set(workerIds)];const {error:delError}=await supabase.from("team_members").delete().eq("team_id",teamId);if(delError)throw delError;if(unique.length){const {error:insError}=await supabase.from("team_members").insert(unique.map(worker_id=>({team_id:teamId,worker_id})));if(insError)throw insError;}setTeams(x=>x.map(t=>t.id===teamId?{...t,members:workers.filter(w=>unique.includes(w.id))}:t))},[workers]);
