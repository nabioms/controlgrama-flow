...
export function StoreProvider({children}:{children:ReactNode}){
 ...
 const setTeamMembers=useCallback(async(teamId:string,workerIds:string[])=>{const unique=[...new Set(workerIds)];const {error:moveError}=unique.length?await supabase.from("team_members").delete().in("worker_id",unique):{error:null};if(moveError)throw moveError;if(unique.length){const {error:insError}=await supabase.from("team_members").insert(unique.map(worker_id=>({team_id:teamId,worker_id})));if(insError)throw insError;}setTeams(x=>x.map(t=>t.id===teamId?{...t,members:workers.filter(w=>unique.includes(w.id))}:{...t,members:(t.members||[]).filter(w=>!unique.includes(w.id))}))},[workers]);
